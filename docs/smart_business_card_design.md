...
### NFC 競合問題への対応 & プライバシー時代の現実

**PN532 のリーダー機能は使わない。** 理由は 2 つ：

1. **アンテナ競合**: PN532 と内蔵 NTAG215 が同じ 13.56 MHz で共振するため、
   PN532 リーダーモードでは自分の NTAG215 を読んでしまう
2. **そもそも取れる情報がない**: 現代のスマホは NFC UID をランダム化、
   個人情報・連絡先・アプリ情報は外部から一切取れない（プライバシー保護のため）

→ **物理 NFC スキャンでは UID（ランダム）すら有用ではない**

代わりに、各交換ごとに**ユニークトークン**を生成し、NTAG215 に書き込む URL に
埋め込む。相手がスマホでアクセスした瞬間にサーバ側でトークンと訪問者情報を
紐づけ、Web 解析で「誰が（IP/UA レベル）」「いつ」「どんな端末で」受け取ったかを
追跡する。

さらに、相手が任意で「私の vCard も送る」ボタンを押した場合のみ、相手の vCard を
受信して同じトークンに紐づける（双方向交換）。

これにより：
- PN532 と NTAG215 のアンテナ競合が起きない（リーダーモードを使わないため）
- 物理的な NFC スキャナーなしで「論理的なスキャナー」が成立
- NFC で取れる以上の情報が Web から取れる（IP/UA/画面/言語/タイムゾーン）
- 法的・プライバシー的にも合法（公開 Web ログ + 相手の明示同意）
