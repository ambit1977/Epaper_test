/*
 * Dual-size Weather Display, LOW-POWER edition
 *
 * Phase 1 software optimizations on top of weather_display_dual:
 *   - WiFi static IP (skip DHCP, ~2s saved)
 *   - WiFi BSSID/channel cached in RTC RAM (fast roam, ~3s saved)
 *   - WiFi.persistent(false) to avoid flash writes
 *   - NTP only once per day (RTC RAM keeps the offset)
 *   - CPU 80MHz outside of WiFi/draw to cut idle current
 *   - Bluetooth disabled, modem sleep enabled
 *   - WiFi.disconnect(true,true) before deep sleep to release power
 *
 * Hardware (same wiring as dual):
 *   BUSY GPIO4, RES GPIO17, D/C GPIO16, CS GPIO23, SCL GPIO5, SDA GPIO18
 *   VCC 3.3V, GND GND
 */
...
// ============================================================================
// RTC RAM — persisted across deep sleep, cleared on power-on reset
// ============================================================================
RTC_DATA_ATTR int     cached_wifi_channel = 0;
RTC_DATA_ATTR uint8_t cached_wifi_bssid[6] = {0};
RTC_DATA_ATTR time_t  ntp_epoch_at_last_sync = 0;     // seconds, last NTP value
RTC_DATA_ATTR uint32_t boot_count = 0;
RTC_DATA_ATTR uint32_t wakes_since_ntp = 0;
// Re-sync NTP every N wakes (e.g. 144 wakes = once per day for 10-min interval)
#define NTP_RESYNC_EVERY_N_WAKES 144
...
bool connectWiFi() {
  ...
  WiFi.persistent(false);  // don't write SSID/PW to flash on every connect
  WiFi.mode(WIFI_STA);
  WiFi.setSleep(true);     // modem sleep when idle

  // Static IP if configured (skip DHCP)
#if STATIC_IP_OCTET_A != 0
  IPAddress local_ip(STATIC_IP_OCTET_A, STATIC_IP_OCTET_B, STATIC_IP_OCTET_C, STATIC_IP_OCTET_D);
  IPAddress gateway(STATIC_IP_OCTET_A, STATIC_IP_OCTET_B, STATIC_IP_OCTET_C, GATEWAY_IP_D);
  IPAddress subnet(255, 255, 255, 0);
  IPAddress dns(STATIC_IP_OCTET_A, STATIC_IP_OCTET_B, STATIC_IP_OCTET_C, DNS_IP_D);
  WiFi.config(local_ip, gateway, subnet, dns);
#endif

  // Fast connect using cached BSSID + channel if we have them
  if (cached_wifi_channel != 0) {
    WiFi.begin(WIFI_SSID, WIFI_PASSWORD, cached_wifi_channel, cached_wifi_bssid);
  } else {
    WiFi.begin(WIFI_SSID, WIFI_PASSWORD);
  }

  // Short timeout — if fast path fails, fall back without cache
  for (int i = 0; i < 60; i++) {  // 6 seconds
    if (WiFi.status() == WL_CONNECTED) {
      // Cache for next time
      cached_wifi_channel = WiFi.channel();
      memcpy(cached_wifi_bssid, WiFi.BSSID(), 6);
  ...
  // Fast path failed — retry without cache once
  if (cached_wifi_channel != 0) {
    Serial.print(" retry");
...
// Format epoch to "HH:MM" JST
String formatTimeJST(time_t now) {
  struct tm t;
  localtime_r(&now, &t);
  char buf[16];
...
// Estimate current epoch from cached NTP value + elapsed wakes
time_t estimateNow() {
  if (ntp_epoch_at_last_sync == 0) return 0;
  return ntp_epoch_at_last_sync + (time_t)(wakes_since_ntp * SLEEP_MINUTES * 60);
}

// Sync NTP only when necessary (first boot or after N wakes)
void syncTime() {
  bool need_sync = (ntp_epoch_at_last_sync == 0) ||
                   (wakes_since_ntp >= NTP_RESYNC_EVERY_N_WAKES);

  if (!need_sync) {
    // Estimate time from cached epoch
    time_t now = estimateNow();
    if (now > 1700000000) {
      // Push into system clock so other code can use time()
      struct timeval tv = { .tv_sec = now, .tv_usec = 0 };
      settimeofday(&tv, nullptr);
      update_time = formatTimeJST(now);
      Serial.println("Time (cached): " + update_time);
      return;
    }
  }
  ...
  Serial.print("NTP");
  for (int i = 0; i < 20; i++) {  // 10 sec max
    time_t now = time(nullptr);
    if (now > 1700000000) {
      ntp_epoch_at_last_sync = now;
      wakes_since_ntp = 0;
      update_time = formatTimeJST(now);
  ...
  Serial.println(" fail");
  // Fall back to estimate if we ever synced before
  time_t now = estimateNow();
  if (now > 1700000000) {
    update_time = formatTimeJST(now);
  } else {
...
void setup() {
  ...
  // --- Disable Bluetooth (we never use it) ---
  btStop();
  esp_bt_controller_disable();

  Serial.begin(115200);
  // Short startup delay so USB serial doesn't miss the first lines on power-on
  delay(500);
  ...
  Serial.println();
  Serial.printf("=== Boot #%lu (wake reason %d) ===\n",
  ...
  if (connectWiFi()) {
    Serial.printf("[time] WiFi: %lu ms\n", millis() - t_wifi);

    unsigned long t_ntp = millis();
    syncTime();
    Serial.printf("[time] NTP: %lu ms\n", millis() - t_ntp);

    unsigned long t_api = millis();
    data_ok = fetchWeather(days);
    Serial.printf("[time] API: %lu ms\n", millis() - t_api);

    // Disconnect and power-down WiFi promptly
    WiFi.disconnect(true, true);
    WiFi.mode(WIFI_OFF);
    esp_wifi_stop();
  } else {
    Serial.printf("[time] WiFi failed: %lu ms\n", millis() - t_wifi);
  }

  unsigned long t_draw = millis();
  if (detected_panel == PANEL_42) {
    drawLayout42();
  } else {
    drawLayout29();
  }
  Serial.printf("[time] Draw: %lu ms\n", millis() - t_draw);

  Serial.printf("[time] Total awake: %lu ms\n", millis() - t_boot);
